            ...
            <div className="flex flex-wrap gap-x-8">
              {allCoursesList?.map((courseData: any, index: number) => {
                if (courseData?.isPurchased) {
                  return <CourseCard courseData={courseData} key={index} isPurchased={courseData?.isPurchased} />;
                } else {
                  return <></>
                }
              })}
            </div>
            <p className="mt-2 text-2xl font-medium">View all <span className="text-[#7E3AF2]">courses</span></p>
            <div className="flex flex-wrap gap-x-8">
              {allCoursesList?.map((courseData: any, index: number) => {
                return <CourseCard courseData={courseData} key={index} isPurchased={courseData?.isPurchased} />;
              })}
